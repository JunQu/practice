...
describe('二叉树的基本遍历', () => {
  it('前序遍历', () => {
    const tree1 = deserialize([1, 2, 3, 4, 5, 6, 7, 8, 9, 0])
    const tree2 = deserialize([1, 2, 3, 4, 5, null, 4, 5])
  ...
  it('后序遍历', () => {
    const tree2 = deserialize([1, 2, 3, 4, 5, 6, 7, 8, 9, 0])
    expect(postorderTraversal(tree2)).toEqual([8, 9, 4, 0, 5, 2, 6, 7, 3, 1])
  })

  it('层序遍历', () => {
    const tree1 = deserialize([3, 9, 20, null, null, 15, 7])
    const tree2 = deserialize([1])
    const tree3 = deserialize([])
...
describe('二叉树的比较', () => {
  it('101 对称二叉树', () => {
    const tree1 = deserialize([1])
    const tree2 = deserialize([1, 2, 2, 3, 4, 4, 3])
    const tree3 = deserialize([1, 2, 2, null, 3, 3, null])

    const tree4 = deserialize([1, 2, 2, null, 3, null, 3])
    const tree5 = deserialize([1, 2, 3])
    const tree6 = deserialize([1, 2, 3, null, 3, null])
...
describe('二叉树基本题目', () => {
  it('652 寻找重复子树', () => {
    const tree1 = deserialize([1, 2, 3, 4, null, 2, 4, null, null, 4])
    const tree2 = deserialize([2, 1, 1])
    const tree3 = deserialize([2, 2, 2, 3, null, 3, null])

    const ans1 = [[4], [2, 4]].map((arr) => deserialize(arr))
    const ans2 = [[1]].map((arr) => deserialize(arr))
    const ans3 = [[2, 3], [3]].map((arr) => deserialize(arr))
